...
BigNumber.config({
  DECIMAL_PLACES: Number(process.env.NEXT_PUBLIC_MAX_BIGNUMBER_EXPONENTIAL_AT),
  EXPONENTIAL_AT: [
    -7,
    Number(process.env.NEXT_PUBLIC_MAX_BIGNUMBER_EXPONENTIAL_AT),
  ],
});
...
export default function CrosschainBridge() {
  ...
  const {
    ...
    (state) => ({
      ...
      tokens: state.tokens,
      ens: state.ens,
      chains_status: state.chains_status,
      routers_status: state.routers_status,
      routers_assets: state.routers_assets,
      sdk: state.sdk,
      rpcs: state.rpcs,
      wallet: state.wallet,
      balances: state.balances,
      gas_prices: state.gas_prices,
    }),
    shallowEqual
  );
  ...
  const { provider, web3_provider, signer, chain_id, address } = {
    ...wallet_data,
  };
  ...
  const [refreshEstimatedFeesSeconds, setRefreshEstimatedFeesSeconds] =
    useState(null);
  ...
  const [tokenApproveResponseCountDown, setTokenApproveResponseCountDown] =
    useState(null);
  ...
  const [activeTransactionTrigger, setActiveTransactionTrigger] =
    useState(null);
  ...
  useEffect(() => {
    ...
    if (
      ...
      Object.keys(swapConfig).length < 1 &&
      chains_data &&
      assets_data
    ) {
      if (
        query.sendingChainId &&
        chains_data.findIndex(
          (c) => !c?.disabled && c.chain_id === Number(query.sendingChainId)
        ) > -1
      ) {
        swapConfig.fromChainId = Number(query.sendingChainId);

        if (
          query.sendingAssetId &&
          assets_data.findIndex(
            (a) =>
              a.contracts?.findIndex(
                (c) =>
                  c.chain_id === swapConfig.fromChainId &&
                  c.contract_address === query.sendingAssetId.toLowerCase()
              ) > -1
          ) > -1
        ) {
          swapConfig.fromAssetId = assets_data.find(
            (a) =>
              a.contracts?.findIndex(
                (c) =>
                  c.chain_id === swapConfig.fromChainId &&
      ...
      if (
        query.receivingChainId &&
        Number(query.receivingChainId) !== swapConfig.fromChainId &&
        chains_data.findIndex(
          (c) => !c?.disabled && c.chain_id === Number(query.receivingChainId)
        ) > -1
      ) {
        ...
        if (
          ...
          query.receivingAssetId &&
          assets_data.findIndex(
            (a) =>
              a.contracts?.findIndex(
                (c) =>
                  c.chain_id === swapConfig.toChainId &&
                  c.contract_address === query.receivingAssetId.toLowerCase()
              ) > -1
          ) > -1
        ) {
          swapConfig.toAssetId = assets_data.find(
            (a) =>
              a.contracts?.findIndex(
                (c) =>
                  c.chain_id === swapConfig.fromChainId &&
    ...
    if (path.includes('from-') && path.includes('to-')) {
      ...
      const asset = assets_data?.find(
        (a) => a.id === assetId || a.symbol?.toLowerCase() === assetId
      );
      const fromContract = asset?.contracts?.find(
        (c) => c?.chain_id === fromChain?.chain_id
      );
      const toContract = asset?.contracts?.find(
        (c) => c?.chain_id === toChain?.chain_id
      );
  ...
  useEffect(() => {
    if (
      asPath &&
      chain_id &&
      (!swapConfig.fromChainId || !swapConfig.toChainId) &&
      swapConfig.toChainId !== chain_id
    ) {
      const query = paramsToObject(
        asPath.indexOf('?') > -1 && asPath.substring(asPath.indexOf('?') + 1)
      );
      if (
        ...
        chains_data?.findIndex(
          (c) => !c?.disabled && c?.chain_id === chain_id
        ) > -1
      ) {
  ...
  useEffect(() => {
    const getData = () => {
      if (
        ...
        !startingSwap &&
        !swapData &&
        !['pending'].includes(tokenApproveResponse?.status)
      ) {
        if (swapConfig.fromChainId) {
          getChainBalances(swapConfig.fromChainId);
        }
  ...
  useEffect(() => {
    const getData = () => {
      if (chains_data) {
        chains_data
          .filter((c) => !c?.disabled)
          .forEach((c) => getChainGasPriceRPC(c.chain_id));
      }
    };
  ...
  useEffect(() => {
    if (swapConfig) {
      ...
      if (
        chains_data?.findIndex(
          (c) => !c?.disabled && c.chain_id === swapConfig.fromChainId
        ) > -1
      ) {
        ...
        if (
          ...
          assets_data?.findIndex(
            (a) =>
              a.id === swapConfig.fromAssetId &&
              a.contracts?.findIndex(
                (c) => c.chain_id === swapConfig.fromChainId
              ) > -1
          ) > -1
        ) {
          _query.sendingAssetId = assets_data
            .find(
              (a) =>
                a.id === swapConfig.fromAssetId &&
                a.contracts?.findIndex(
      ...
      if (
        chains_data?.findIndex(
          (c) => !c?.disabled && c.chain_id === swapConfig.toChainId
        ) > -1
      ) {
        ...
        if (
          ...
          assets_data?.findIndex(
            (a) =>
              a.id === swapConfig.toAssetId &&
              a.contracts?.findIndex(
                (c) => c.chain_id === swapConfig.toChainId
              ) > -1
          ) > -1
        ) {
          _query.receivingAssetId = assets_data
            .find(
              (a) =>
                a.id === swapConfig.toAssetId &&
                a.contracts?.findIndex(
      ...
      if (Object.keys(_query).length > 0) {
        const fromChain = chains_data?.find(
          (c) => c?.chain_id === swapConfig.fromChainId
        );
        const toChain = chains_data?.find(
          (c) => c?.chain_id === swapConfig.toChainId
        );
        const fromAsset = assets_data?.find(
          (a) => a?.id === swapConfig.fromAssetId
        );
        const toAsset = assets_data?.find(
          (a) => a?.id === swapConfig.toAssetId
        );
        ...
        router.push(
          `/${
            fromChain && toChain
              ? `${
                  fromAsset || toAsset
                    ? `${(fromAsset || toAsset).symbol}-`
                    : ''
                }from-${fromChain.id}-to-${toChain.id}`
              : ''
          }${
  ...
  useEffect(() => {
    if (
      tokenApproved === true ||
      (tokenApproved && tokenApproved?.toNumber() >= constants.MaxUint256)
    ) {
  ...
  const reset = async (is_from_address, is_from_switch_protocol) => {
    const isReset =
      !is_from_address ||
      (address &&
        swapData?.prepareResponse?.from?.toLowerCase() !==
          address.toLowerCase());
  ...
  const getDomains = async (evmAddresses) => {
    evmAddresses = (
      Array.isArray(evmAddresses) ? evmAddresses : [evmAddresses]
    ).filter((a) => a);
    ...
    if (evmAddresses.length > 0) {
      ...
      for (let i = 0; i < addressChunk.length; i++) {
        const domainsResponse = await domains({
          where: `{ resolvedAddress_in: [${addressChunk[i]
      ...
      if (ensData?.length > 0) {
        ...
        for (let i = 0; i < evmAddresses.length; i++) {
          ...
          const resolvedAddresses = ensData.filter(
            (d) => d?.resolvedAddress?.id?.toLowerCase() === evmAddress
          );
        ...
        dispatch({
          ...
          value: Object.fromEntries(
            ensData
              .filter(
                (d) =>
                  !ensResponses?.[d?.resolvedAddress?.id?.toLowerCase()]
                    ?.reverseRecord ||
                  d?.name ===
                    ensResponses?.[d?.resolvedAddress?.id?.toLowerCase()]
                      .reverseRecord
  ...
  const getChainBalancesMulticall = async (chain_id, contract_addresses) => {
    ...
    const rpcUrls =
      chains_data
        ...
        .flatMap((c) => c?.provider_params?.flatMap((p) => p?.rpcUrls || [])) ||
      [];
  ...
  const getChainTokenMulticall = async (chain_id, contracts) => {
    if (chain_id && contracts) {
      const balances = await getChainBalancesMulticall(
        chain_id,
        ...
        for (let i = 0; i < balances.length; i++) {
          ...
          if (balance && contract) {
            const _balance = BigNumber(balance)
              .shiftedBy(-contract.contract_decimals)
              .toNumber();
            assets.push({
              ...contract,
              amount: _balance,
  ...
  const getChainTokenRPC = async (chain_id, contract) => {
    if (chain_id && contract) {
      const balance = await getChainBalanceRPC(
        chain_id,
        contract.contract_address
      );

      if (balance) {
        const _balance = BigNumber(balance.toString())
          .shiftedBy(-contract.contract_decimals)
          .toNumber();
        const asset = {
          ...contract,
    ...
    if (chain_id && address) {
      ...
      if (!balances_data?.[chain_id]) {
        if (multicallAddress && ![1666600000, 192837465].includes(chain_id)) {
          getChainTokenMulticall(
            chain_id,
            contracts?.filter(
              (c) => c.contract_address !== constants.AddressZero
            )
          );
          contracts
            .filter((c) => c.contract_address === constants.AddressZero)
            .forEach((c) => getChainTokenRPC(chain_id, c));
        } else {
  ...
  const getChainBalance = (chain_id, side = 'from') => {
    ...
    const asset = assets_data?.find(
      (a) => a?.id === swapConfig[`${side}AssetId`]
    );
    return balances_data?.[chain_id]?.find(
      (c) =>
        c?.contract_address ===
        asset?.contracts?.find((_c) => _c?.chain_id === chain_id)
          ?.contract_address
    );
  };
  ...
  const isSupport = () => {
    ...
    return (
      ...
      !(
        swapConfig.fromChainId &&
        fromAsset.contracts?.findIndex(
          (c) => c?.chain_id === swapConfig.fromChainId
        ) < 0
      ) &&
      !(
        swapConfig.toChainId &&
        toAsset.contracts?.findIndex(
          (c) => c?.chain_id === swapConfig.toChainId
        ) < 0
      )
    );
  ...
  const getChainSynced = (chain_id) =>
    !chains_status_data ||
    chains_status_data.find((c) => c.chain_id === chain_id)?.synced;
  ...
  const isTokenApproved = async (is_after_approve) => {
    ...
    if (
      ...
      swapConfig.fromAssetId &&
      isSupport() &&
      (is_after_approve || !tokenApproveResponse)
    ) {
      ...
      if (contract?.contract_address) {
        if (contract.contract_address === constants.AddressZero) {
          ...
          approved = await getApproved(
            signer,
            contract.contract_address,
            getDeployedTransactionManagerContract(swapConfig.fromChainId)
              ?.address
          );
        }
      }
  ...
  const approveToken = async () => {
    ...
    setTokenApproveResponse(null);
    try {
      const tx_approve = await approve(
        signer,
        contract?.contract_address,
        getDeployedTransactionManagerContract(swapConfig.fromChainId)?.address,
        infiniteApproval
          ? constants.MaxUint256
          : BigNumber(swapConfig.amount)
              .decimalPlaces(6, BigNumber.ROUND_CEIL)
              .shiftedBy(contract?.contract_decimals)
              .toString()
      );
      const tx_hash = tx_approve?.hash;
      setTokenApproveResponse({
        status: 'pending',
        message: `Wait for ${
  ...
  const getChainPrepareGasFee = async (chain_id) => {
    ...
    if (chain_id && rpcs_data?.[chain_id]) {
      ...
      if (chain) {
        ...
        const provider_url = _.head(
          rpcs_data[chain_id].providerConfigs?.map(
            (p) => p?.provider?.connection?.url
          )
        );
        const web3 = new Web3(provider_url);
        const gasPrice = await web3.eth.getGasPrice();
        gasFee = BigNumber(gasPrice)
          .multipliedBy(gasLimit)
          .shiftedBy(-chain.provider_params?.[0]?.nativeCurrency?.decimals)
          .toNumber();
      }
    }
  ...
  const estimate = async (controller) => {
    if (isSupport() && !swapData) {
      ...
      const fromContract = fromAsset?.contracts?.find(
        (c) => c?.chain_id === swapConfig.fromChainId
      );
      const toContract = toAsset?.contracts?.find(
        (c) => c?.chain_id === swapConfig.toChainId
      );
            ...
            if (!controller.signal.aborted) {
              ...
              try {
                const response = await sdk_data.getTransferQuote({
                  ...
                  receivingAddress:
                    advancedOptions?.receiving_address || address,
                  amount: BigNumber(swapConfig.amount)
                    .shiftedBy(fromContract.contract_decimals)
                    .toString(),
                  transactionId,
                  ...
                  preferredRouters:
                    advancedOptions?.preferred_router?.length > 0
                      ? advancedOptions.preferred_router.split(',')
                      : undefined,
                  dryRun: false,
                });
                if (!controller.signal.aborted) {
                  if (
                    response?.bid?.sendingChainId === swapConfig.fromChainId &&
                    response?.bid?.receivingChainId === swapConfig.toChainId &&
                    response?.bid?.sendingAssetId ===
                      fromContract.contract_address
                  ) {
                    getDomains([
                      address,
                      response?.bid?.router,
                      advancedOptions?.receiving_address,
                    ]);
                    const relayerFee = BigNumber(
                      response?.metaTxRelayerFee || 0
                    )
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber();
                    const gasFee = BigNumber(
                      response?.gasFeeInReceivingToken || 0
                    )
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber();
                    const routerFee = BigNumber(response?.routerFee || 0)
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber();
                    // const totalFee = BigNumber(response?.totalFee || 0).shiftedBy(-toContract.contract_decimals).toNumber()
                    /* hotfix */
                    const totalFee =
                      BigNumber(response?.bid?.amount || 0)
                        .shiftedBy(-fromContract.contract_decimals)
                        .toNumber() -
                      BigNumber(response?.bid?.amountReceived || 0)
                        .shiftedBy(-toContract.contract_decimals)
                        .toNumber() +
                      relayerFee;
                    setFees({
          ...
          if (sdk_data) {
            if (!controller.signal.aborted) {
              ...
              try {
                ...
                if (!controller.signal.aborted) {
                  setFees({
                    relayer: BigNumber(response?.relayerFee || 0)
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber(),
                    gas: BigNumber(response?.gasFee || 0)
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber(),
                    router: BigNumber(response?.routerFee || 0)
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber(),
                    total: BigNumber(response?.totalFee || 0)
                      .shiftedBy(-toContract.contract_decimals)
                      .toNumber(),
                    prepareGasFee:
                      fromContract?.contract_address ===
                        constants.AddressZero &&
                      (await getChainPrepareGasFee(swapConfig.fromChainId)),
                  });
                }
              } catch (error) {
                if (!controller.signal.aborted) {
  ...
  const swap = async () => {
    ...
    if (sdk_data) {
      try {
        const response = await sdk_data.prepareTransfer(
          estimatedAmount,
          infiniteApproval
        );
        setSwapData({
          ...response,
          sendingChainId: estimatedAmount?.bid?.sendingChainId,
          receivingChainId: estimatedAmount?.bid?.receivingChainId,
        });
        setSwapResponse(null);
      } catch (error) {
        setSwapResponse({
  ...
  const swapNomad = async (
    ...
    try {
      ...
      setSwapResponse({
        status: 'success',
        message: 'Sending through NOMAD',
        hash: response?.dispatch?.transactionHash,
      });
    } catch (error) {
      setSwapResponse({
        status: 'failed',
  ...
  const isBreakAll = (message) =>
    ['code=', ' 0x'].findIndex((p) => message?.includes(p)) > -1;
  ...
  const tokenPrice = tokens_data?.find(
    (t) =>
      (t.chain_id === fromChain?.chain_id &&
        t.contract_address === fromContract?.contract_address &&
        typeof t.price === 'number') ||
      (t.chain_id === toChain?.chain_id &&
        t.contract_address === toContract?.contract_address &&
        typeof t.price === 'number')
  )?.price;

  const confirmFromChain = chains_data?.find(
    (c) => c?.chain_id === estimatedAmount?.bid?.sendingChainId
  );
  const confirmToChain = chains_data?.find(
    (c) => c?.chain_id === estimatedAmount?.bid?.receivingChainId
  );
  const confirmFromAsset = assets_data?.find(
    (a) =>
      a?.contracts?.findIndex(
        (c) =>
          c.chain_id === estimatedAmount?.bid?.sendingChainId &&
          c.contract_address?.toLowerCase() ===
            estimatedAmount?.bid?.sendingAssetId?.toLowerCase()
      ) > -1
  );
  const confirmToAsset = assets_data?.find(
    (a) =>
      a?.contracts?.findIndex(
        (c) =>
          c.chain_id === estimatedAmount?.bid?.receivingChainId &&
          c.contract_address?.toLowerCase() ===
            estimatedAmount?.bid?.receivingAssetId?.toLowerCase()
      ) > -1
  );
  const confirmFromContract = confirmFromAsset?.contracts?.find(
    (c) => c?.chain_id === estimatedAmount?.bid?.sendingChainId
  );
  const confirmToContract = confirmToAsset?.contracts?.find(
    (c) => c?.chain_id === estimatedAmount?.bid?.receivingChainId
  );
  const confirmTokenPrice = tokens_data?.find(
    (t) =>
      (t.chain_id === confirmToChain?.chain_id &&
        t.contract_address === confirmToContract?.contract_address &&
        typeof t.price === 'number') ||
      (t.chain_id === confirmToChain?.chain_id &&
        t.contract_address === confirmToContract?.contract_address &&
        typeof t.price === 'number')
  ...
  const confirmAmount =
    confirmFromContract &&
    estimatedAmount?.bid?.amount &&
    BigNumber(estimatedAmount.bid.amount)
      .shiftedBy(-confirmFromContract.contract_decimals)
      .toNumber();
  const confirmRelayerFee =
    confirmToContract &&
    estimatedAmount &&
    BigNumber(estimatedAmount.metaTxRelayerFee || 0)
      .shiftedBy(-confirmToContract.contract_decimals)
      .toNumber();
  const confirmGasFee =
    confirmToContract &&
    estimatedAmount &&
    BigNumber(estimatedAmount.gasFeeInReceivingToken || 0)
      .shiftedBy(-confirmToContract.contract_decimals)
      .toNumber();
  const confirmRouterFee =
    confirmToContract &&
    estimatedAmount &&
    BigNumber(estimatedAmount.routerFee || 0)
      .shiftedBy(-confirmToContract.contract_decimals)
      .toNumber();
  const confirmAmountReceived =
    confirmToContract &&
    estimatedAmount?.bid?.amountReceived &&
    BigNumber(estimatedAmount.bid.amountReceived)
      .shiftedBy(-confirmToContract.contract_decimals)
      .toNumber() - confirmRelayerFee;
  ...
  const confirmFees =
    confirmToContract &&
    estimatedAmount &&
    confirmAmount - confirmAmountReceived;
  const estimatedFees =
    typeof confirmFees === 'number' ? confirmFees : fees?.total;
  /* hotfix */
        ...
        title={
          <div className='flex items-center justify-between space-x-2.5'>
            ...
            <span className='font-mono space-x-1'>
              ...
              <span>
                {(estimatedAmount ? confirmToContract : toContract)?.symbol ||
                  (estimatedAmount ? confirmToAsset : toAsset)?.symbol}
              </span>
            </span>
          </div>
        }
        content={
          <div className='flex flex-col space-y-1'>
            <div className='flex items-center justify-between space-x-2.5'>
              ...
              <span className='font-mono'>
                {typeof (estimatedAmount
                  ...
                  : fees?.relayer) === 'number'
                  ? `${estimatedAmount ? '' : '~'}${numberFormat(
                      estimatedAmount ? confirmRelayerFee : fees.relayer,
                      '0,0.000000',
                      true
                    )}`
                  : 'n/a'}
            ...
            <div className='flex items-center justify-between space-x-2.5'>
              ...
              <span className='font-mono'>
                {typeof (estimatedAmount ? confirmGasFee : fees?.gas) ===
                'number'
                  ? `${estimatedAmount ? '' : '~'}${numberFormat(
                      estimatedAmount ? confirmGasFee : fees.gas,
                      '0,0.000000',
                      true
                    )}`
                  : 'n/a'}
            ...
            <div className='flex items-center justify-between space-x-2.5'>
              ...
              <span className='font-mono'>
                {typeof (estimatedAmount ? confirmRouterFee : fees?.router) ===
                'number'
                  ? `${estimatedAmount ? '' : '~'}${numberFormat(
                      estimatedAmount ? confirmRouterFee : fees.router,
                      '0,0.000000',
                      true
                    )}`
  ...
  const maxTransfers =
    ...
    _.orderBy(
      Object.values(
        _.groupBy(
          routers_assets_data.flatMap(
            (ra) =>
              ra?.asset_balances?.filter(
                (ab) => ab?.chain?.chain_id === toChain?.chain_id
              ) || []
          ),
          'assetId'
        )
        ...
        if (a && routers_status_data) {
          assets_from_chains = Object.fromEntries(
            chains_data
              ?.filter((c) => !c.disabled)
              .map((c) => {
                const assets = a.filter(
                  (_a) =>
                    routers_status_data?.findIndex(
                      (r) =>
                        r?.routerAddress?.toLowerCase() ===
                          _a?.router?.id?.toLowerCase() &&
                        r?.supportedChains?.includes(c?.chain_id) &&
                        r?.supportedChains?.includes(toChain?.chain_id) &&
                        r?.supportedChains?.includes(fromChain?.chain_id)
                    ) > -1
                );
  ...
  const fromBalanceComponent = address &&
    swapConfig.fromChainId &&
    fromAsset && (
      <div
        className={`flex items-center font-mono ${
          fromBalanceAmount > 0
            ? 'text-green-500 dark:text-white font-semibold'
      ...
      >
        {balances_data?.[swapConfig.fromChainId] ? (
          <>
            <span>
              {fromBalance
                ? numberFormat(fromBalanceAmount, '0,0.000000', true)
                : 'n/a'}
            </span>
            {fromBalance && (
              <span>
    ...
    >
      {balances_data?.[swapConfig.toChainId] ? (
        <>
          <span>
            {toBalance
              ? numberFormat(toBalanceAmount, '0,0.000000', true)
              : 'n/a'}
          </span>
          {toBalance && (
            <span>
              {toBalance?.symbol || toContract?.symbol || toAsset?.symbol}
            </span>
          )}
        </>
      ) : (
  ...
  const maxBalanceAmount =
    fromBalanceAmount > minAmount ? fromBalanceAmount : 0;
  let maxTransfer = maxTransfers?.find(
    (t) =>
      t?.chain?.chain_id === swapConfig.toChainId &&
      t?.contract_address === toContract?.contract_address
  );
  if (maxTransfer?.assets_from_chains?.[swapConfig.fromChainId]) {
    maxTransfer.amount =
      maxTransfer.assets_from_chains[swapConfig.fromChainId].amount;
    maxTransfer.amount_value =
      maxTransfer.assets_from_chains[swapConfig.fromChainId].amount_value;
  } else if (swapConfig.fromChainId) {
    maxTransfer = null;
  }
  let maxAmount = maxTransfer
    ? maxTransfer.amount < maxBalanceAmount
      ? maxTransfer.amount > minAmount
        ? maxTransfer.amount
        : 0
      : maxBalanceAmount
    : 0;
  if (maxAmount) {
    ...
    maxAmount = BigNumber(maxAmount)
      .decimalPlaces(6, BigNumber.ROUND_FLOOR)
      .toNumber();
  }
  const isExceedMaxLiquidity =
    typeof maxTransfer?.amount !== 'number' ||
    (typeof swapConfig.amount === 'number' &&
      swapConfig.amount > maxTransfer.amount);

  const supportNomad =
    swapConfig.fromChainId &&
    swapConfig.toChainId &&
    fromAsset?.nomad_support?.findIndex(
      (p) =>
        p?.from_chain_id === swapConfig.fromChainId &&
        p?.to_chain_id === swapConfig.toChainId
    ) > -1 &&
    toAsset?.nomad_support?.findIndex(
      (p) =>
        p?.from_chain_id === swapConfig.fromChainId &&
        p?.to_chain_id === swapConfig.toChainId
    ) > -1;
  ...
  const nomadUrl =
    useNomad &&
    (fromChain.optional_bridge_urls?.find((url) => url?.includes('.nomad.')) ||
      toChain.optional_bridge_urls?.find((url) => url?.includes('.nomad.')));

  const mustApproveToken =
    isSupport() &&
    fromContract &&
    !(
      tokenApproved &&
      (typeof tokenApproved === 'boolean'
        ? tokenApproved
        : tokenApproved.gte(
            BigNumber(swapConfig.amount).shiftedBy(
              fromContract.contract_decimals
            )
          ))
    );
  const unlimitAllowance =
    tokenApproved === true ||
    (tokenApproved && tokenApproved?.toNumber() >= constants.MaxUint256);
  const actionDisabled =
    tokenApproveResponse?.status === 'pending' || startingSwap;
    ...
    <>
      ...
      >
        ...
        <span className='normal-case font-medium'>
          {infiniteApproval || unlimitAllowance ? 'Infinite' : 'Exact'}
        </span>
      </div>
      <Switch
  ...
  const fromChainSynced = getChainSynced(swapConfig.fromChainId);
  const toChainSynced = getChainSynced(swapConfig.toChainId);
  const unsyncedChains = [
    !fromChainSynced && fromChain,
    !toChainSynced && toChain,
  ].filter((c) => c);

  const receivingAddress =
    useNomad || !estimatedAmount?.bid?.receivingAddress
      ? advancedOptions?.receiving_address || address
      : estimatedAmount.bid.receivingAddress;
  const mustChangeChain =
    swapConfig.fromChainId &&
    ...
    <>
      {tokenApproveResponse && (
        <Notification
          ...
          innerClassNames={`${
            tokenApproveResponse.status === 'failed'
            ...
            <span className='flex flex-wrap items-center'>
              ...
              {fromChain?.explorer?.url && tokenApproveResponse.tx_hash && (
                <a
                  ...
                  }${fromChain.explorer.transaction_path?.replace(
                    '{tx}',
                    tokenApproveResponse.tx_hash
                  )}`}
                  target='_blank'
              ...
              {['success'].includes(tokenApproveResponse.status) &&
                typeof tokenApproveResponseCountDown === 'number' && (
                  <span className='font-mono'>
                    (close in {tokenApproveResponseCountDown}s)
                  </span>
                )}
            </span>
          }
      ...
      <div className='grid grid-flow-row grid-cols-1 lg:grid-cols-8 items-start gap-4'>
        ...
        <div className='col-span-1 lg:col-span-4'>
          <div className='space-y-4 mt-8'>
            ...
            {chains_status_data
              ?.filter((c) => !c.disabled && !c.synced)
              .filter(
                (c) =>
                  [swapConfig.fromChainId, swapConfig.toChainId].filter(
                    (_c) => _c
                  ).length < 1 ||
                  [swapConfig.fromChainId, swapConfig.toChainId].includes(
                    c.chain_id
                  )
              ).length > 0 && (
              <Alert
                ...
                icon={
                  <TiWarning className='w-4 sm:w-6 h-4 sm:h-6 stroke-current mr-3' />
                }
                closeDisabled={true}
                rounded={true}
                className='items-start'
              >
                <div className='block leading-4 text-xs xl:text-base font-medium'>
                  <span className='mr-1.5'>
                    ...
                    may be delayed temporarily. Funds are always safe! If you
                    have active transactions, check their status
                  </span>
                  <a
          ...
          <div className='flex flex-col items-center justify-center space-y-4 sm:space-y-6 my-6'>
            <div className='w-full max-w-lg space-y-2'>
              <div className='flex items-center justify-between space-x-2'>
                <div>
                  <h1 className='uppercase text-base sm:text-lg font-semibold'>
                    Cross-Chain Transfer
                  </h1>
                  {asPath?.includes('from-') &&
                    asPath?.includes('to-') &&
                    headMeta?.title && (
                      <h2 className='text-gray-400 dark:text-gray-600 text-xs sm:text-sm'>
              ...
              <div className='bg-white dark:bg-gray-900 rounded-xl shadow-md space-y-8 sm:space-y-6 p-8 sm:p-6'>
                <div className='grid grid-flow-row grid-cols-1 sm:grid-cols-5 gap-6'>
                  <div className='sm:col-span-2 flex flex-col items-center sm:items-start'>
                    <div className='w-48 flex items-center justify-center space-x-1.5'>
                      <span className='text-gray-400 dark:text-gray-600 text-lg font-medium text-center'>
                        From
                      </span>
                          ...
                          content={
                            <div className='flex flex-col'>
                              <div className='flex items-center space-x-1'>
                                ...
                                <span className='font-semibold'>
                                  {
                                    gas_prices_data[fromChain.chain_id]
                                      .gas_price
                                  }{' '}
                                  Gwei
                                </span>
                      ...
                      onSelect={(asset_id) => {
                        setSwapConfig({
                          ...
                          toAssetId: !swapConfig.toAssetId
                            ...
                            ? swapConfig.fromAssetId === asset_id
                              ? null
                              : swapConfig.fromAssetId
                            : asset_id, // swapConfig.toAssetId,
                          amount:
                  ...
                  <div className='flex items-center justify-center'>
                    ...
                    >
                      <MdSwapVerticalCircle
                        size={40}
                        className='sm:hidden rounded-full shadow text-blue-400 hover:text-blue-600 dark:text-gray-700 dark:hover:text-white'
                      />
                      <MdSwapHorizontalCircle
                        size={40}
                        className='hidden sm:block rounded-full shadow text-blue-400 hover:text-blue-600 dark:text-gray-700 dark:hover:text-white'
                      />
                    </button>
                  </div>
                              ...
                              <div className='flex items-center space-x-1'>
                                ...
                                <span className='font-semibold'>
                                  {Number(
                                    moment().diff(
                                      moment(
                                        gas_prices_data[toChain.chain_id]
                                          .updated_at
                                      ),
                                      'second'
                                    )
                      ...
                      {protocols.map((p, i) => (
                        ...
                        >
                          <button
                            ...
                            className={`${
                              p.id === bridgeProtocol
                                ? 'ring-2 ring-blue-600 dark:ring-white'
                                : ''
                            } rounded-xl shadow py-1 px-2`}
                          >
                            <img
                              src={p.image?.[theme]}
                              alt=''
                          ...
                          <Popover
                            ...
                            title={
                              <div className='flex items-center justify-between space-x-2.5'>
                                ...
                                <span className='font-mono'>
                                  {fromContract?.symbol || fromAsset?.symbol}
                                </span>
                              </div>
                            }
                            content={
                              <div className='flex flex-col space-y-1'>
                                {fees &&
                                  fromContract?.contract_address ===
                                    ...
                                    <div className='flex items-center justify-between space-x-2.5'>
                                      ...
                                      <span className='font-mono'>
                                        {typeof fees.prepareGasFee === 'number'
                                          ? numberFormat(
                                              fees.prepareGasFee,
                                              fees.prepareGasFee > 100
                                                ? '0,0.00'
                                                : '0,0.000000',
                                              true
                                            )
                                ...
                                <div className='flex items-center justify-between space-x-2.5'>
                                  ...
                                  <span className='font-mono'>
                                    {typeof fromBalanceAmount === 'number'
                                      ? numberFormat(
                                          fromBalanceAmount,
                                          fromBalanceAmount > 1000
                                            ? '0,0.00'
                                            : '0,0.000000',
                                          true
                                        )
                    ...
                    <div className='grid grid-flow-row grid-cols-1 sm:grid-cols-5 gap-6 sm:mx-3'>
                      ...
                      <div className='sm:col-span-3 flex items-center justify-center sm:justify-end'>
                        ...
                        typeof estimatedFees !== 'number' ? (
                          ...
                          <div className='flex flex-col items-center sm:items-end space-y-1'>
                            {feesPopover(
                              <span className='flex items-center font-mono font-semibold space-x-1.5'>
                                ...
                                <span>
                                  {toContract?.symbol || toAsset?.symbol}
                                </span>
                                {!estimatedAmount && (
                          ...
                          <div className='font-mono text-gray-400 dark:text-gray-600'>
                            n/a
                          </div>
                        )}
                      </div>
                    ...
                    <div className='grid grid-flow-row grid-cols-1 sm:grid-cols-5 gap-6 sm:mx-3'>
                      ...
                      <div className='sm:col-span-3 flex flex-col items-center sm:items-end'>
                        <div className='flex items-center justify-center sm:justify-start font-mono font-semibold space-x-1.5'>
                          ...
                          <span>
                            {confirmToContract?.symbol ||
                              confirmToAsset?.symbol ||
                              toContract?.symbol ||
                              toAsset?.symbol}
                          </span>
                        </div>
                        {!estimatedAmountResponse &&
                          !estimatingAmount &&
                          estimatedAmount &&
                          typeof confirmTokenPrice === 'number' && (
                            <div className='font-mono text-green-500 text-xs font-medium'>
                              ({currency_symbol}
                              {numberFormat(
                                confirmAmountReceived * confirmTokenPrice,
                                '0,0.00'
                              )}
                              )
                            </div>
                    ...
                    <div className='grid grid-flow-row grid-cols-2 sm:grid-cols-5 gap-6 sm:mx-3'>
                      <div className='sm:col-span-2 flex items-center justify-start space-x-2'>
                        ...
                        <Popover
                          ...
                          content={
                            <div className='w-72'>
                              Only the exact amount is allowed to be
                              transferred. You will need to reapprove for a
                      ...
                      <div className='sm:col-span-3 flex items-center justify-end space-x-1.5'>
                        {unlimitAllowance ? (
                          <Popover
                            ...
                            content={
                              <div className='w-36'>Infinite allowance on</div>
                            }
                            titleClassName='normal-case py-1'
                          >
                ...
                />
                {isSupport() &&
                (swapData || balances_data?.[swapConfig.fromChainId]) &&
                (typeof swapConfig.amount === 'number' ||
                  ...
                  mustChangeChain && web3_provider ? (
                    <Wallet
                      ...
                      buttonDisconnectTitle={
                        <>
                          <span>
                            {isWalletConnect ? 'Reconnect' : 'Switch'} to
                          </span>
                          <Img
                            src={fromChain?.image}
                            alt=''
                    ...
                    >
                      <span>
                        Not enough bridge liquidity to complete transfer. Use a
                        smaller amount.
                      </span>
                    </Alert>
                    ...
                    >
                      <span>
                        Send at least{' '}
                        <span className='font-mono font-semibold mx-1.5'>
                          {numberFormat(estimatedFees, '0,0.000000', true)}{' '}
                          {toContract?.symbol || toAsset?.symbol}
                        </span>{' '}
                        to cover fees.
                      </span>
                    ...
                    check_balances &&
                    maxBalanceAmount < swapConfig.amount ? (
                    <Alert
                      color='bg-red-400 dark:bg-red-500 text-white text-base'
                      icon={
                        <BiMessageError className='w-4 sm:w-6 h-4 sm:h-6 stroke-current mr-3' />
                      ...
                      >
                        <div className='flex items-center justify-between space-x-1'>
                          <div className='flex items-center space-x-2'>
                            <span
                              className={`break-${
                                isBreakAll(swapResponse.message)
                                  ? 'all'
                                  : 'words'
                              }`}
                            >
                            ...
                            {['success'].includes(swapResponse.status) &&
                              swapResponse.hash &&
                              nomadUrl && (
                                <a
                      ...
                      <ModalConfirm
                        ...
                        buttonTitle={
                          <>
                            ...
                            <Img
                              src={
                                protocols.find((p) => p.id === 'nomad')?.image
                                  ?.dark
                              }
                              alt=''
                            ...
                            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-1 sm:space-y-0 sm:space-x-2 xl:space-x-2.5'>
                              ...
                              {receivingAddress && (
                                <div className='flex items-center space-x-1.5 sm:space-x-1 xl:space-x-1.5'>
                                  {ens_data?.[receivingAddress.toLowerCase()]
                                    ?.name && (
                                    <Img
                                      src={`${
                                        process.env.NEXT_PUBLIC_ENS_AVATAR_URL
                                      }/${
                                        ens_data?.[
                                  ...
                                  {toChain?.explorer?.url && (
                                    <a
                                      href={`${
                                        toChain.explorer.url
                                      }${toChain.explorer.address_path?.replace(
                                        '{address}',
                                        receivingAddress
                                      )}`}
                                      target='_blank'
                        ...
                        confirmButtonTitle={
                          <span className='flex items-center justify-center space-x-1.5 py-2'>
                            {startingSwap && (
                              <TailSpin
                                color={theme === 'dark' ? 'white' : 'white'}
                                width='20'
                    ...
                    >
                      <span>
                        {unsyncedChains.map((c, i) => (
                          <span
                            key={i}
                            className='inline-flex items-baseline mr-1.5'
                          >
                            <Img
                              src={c.image}
                              alt=''
                        ...
                        <span>
                          subgraph{unsyncedChains.length > 1 ? 's' : ''} is out
                          of sync. Please try again later.
                        </span>
                      </span>
                    </Alert>
                  ) : activeTransactionOpen ? null : !swapData &&
                    !swapResponse &&
                    !estimatedAmountResponse &&
                    (estimatedAmount || estimatingAmount) ? (
                    <div>
                      ...
                      {estimatingAmount ? (
                        <button
                          ...
                          } rounded-2xl flex items-center justify-center text-white text-xs sm:text-lg space-x-1.5 sm:space-x-2 py-3 sm:py-4 px-2 sm:px-3`}
                        >
                          {estimatingAmount ? (
                            <>
                              <TailSpin
                          ...
                          {estimatingAmount &&
                            typeof bidIntervalSeconds === 'number' && (
                          ...
                          <button
                            ...
                            className={`w-full ${
                              actionDisabled
                                ? 'bg-blue-400 dark:bg-blue-500 cursor-not-allowed'
                                : 'bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700'
                            } rounded-2xl flex items-center justify-center text-white text-base sm:text-lg space-x-2 py-3 sm:py-4 px-2 sm:px-3`}
                          >
                            {tokenApproveResponse?.status === 'pending' ? (
                              <>
                                <TailSpin
                            ...
                            <div className='flex flex-col space-y-4 -mb-2'>
                                  ...
                                  <div className='flex items-center space-x-1.5 sm:space-x-1 xl:space-x-1.5'>
                                    ...
                                    {confirmToChain?.explorer?.url && (
                                      <a
                                        href={`${
                                          confirmToChain.explorer.url
                                        }${confirmToChain.explorer.address_path?.replace(
                                          '{address}',
                                          receivingAddress
                                        )}`}
                                        target='_blank'
                              ...
                              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-1 sm:space-y-0 sm:space-x-2 xl:space-x-2.5'>
                                ...
                                <div className='flex items-center space-x-1.5 sm:space-x-1 xl:space-x-1.5'>
                                  <span className='text-base sm:text-xs xl:text-base font-semibold'>
                                    {ens_data?.[
                                      estimatedAmount.bid?.router?.toLowerCase()
                                    ]?.name ||
                                      ellipseAddress(
                                        estimatedAmount.bid?.router?.toLowerCase(),
                                        10
                                  ...
                                  <Copy
                                    size={18}
                                    text={estimatedAmount.bid?.router}
                                  />
                                  <a
                                    href={`${
                                      process.env.NEXT_PUBLIC_EXPLORER_URL
                                    }/router/${estimatedAmount.bid?.router?.toLowerCase()}`}
                                    target='_blank'
                                    rel='noopener noreferrer'
                                    className='text-blue-600 dark:text-white'
                                  >
                              ...
                              <div className='flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-1 sm:space-y-0 sm:space-x-2 xl:space-x-2.5'>
                                ...
                                <div className='sm:text-right'>
                                  <div className='font-mono text-lg font-semibold space-x-1.5'>
                                    ...
                                    <span>
                                      {confirmFromContract?.symbol ||
                                        confirmFromAsset?.symbol}
                                    </span>
                                  </div>
                                  {confirmAmount &&
                                    typeof confirmTokenPrice === 'number' && (
                                      <div className='font-mono text-blue-500 sm:text-right'>
                                        ({currency_symbol}
                                        {numberFormat(
                                          confirmAmount * confirmTokenPrice,
                                          '0,0.00'
                                        )}
                                        )
                                      </div>
                                            ...
                                            <div className='w-full grid grid-flow-row grid-cols-2 gap-1.5'>
                                              <span className='flex items-center text-gray-400 dark:text-gray-600 text-sm mr-4'>
                                                ...
                                                <Popover
                                                  ...
                                                  content={
                                                    <div className='w-48 text-xs'>
                                                      Fee for relayer to deliver
                                                      funds to user on receiving
                                                      chain.
                                                    </div>
                                                  }
                                                  titleClassName='normal-case py-1'
                                                >
                                                  <BsQuestionCircle
                                                    size={14}
                                                    className='text-gray-400 dark:text-gray-600 ml-1.5'
                                                  />
                                                </Popover>
                                              </span>
                                              ...
                                              <span className='flex items-center text-gray-400 dark:text-gray-600 text-sm mr-4'>
                                                ...
                                                <Popover
                                                  ...
                                                  content={
                                                    <div className='w-48 text-xs'>
                                                      Covers gas expense for
                                                      router transactions on
                                    ...
                                    {estimatedFees &&
                                      typeof confirmTokenPrice === 'number' && (
                                        <div className='font-mono text-red-500 sm:text-right'>
                                          ({currency_symbol}
                                          {numberFormat(
                                            estimatedFees * confirmTokenPrice,
                                            '0,0.00'
                                          )}
                                          )
                                        </div>
                    ...
                    <Alert
                      color={`${
                        estimatedAmountResponse.status === 'failed'
                          ? 'bg-red-400 dark:bg-red-500'
                          : estimatedAmountResponse.status === 'success'
                          ? 'bg-green-400 dark:bg-green-500'
                          : 'bg-blue-400 dark:bg-blue-500'
                      } text-white text-base`}
                      icon={
                    ...
                    >
                      <div className='flex items-center justify-between space-x-2'>
                        <span
                          className={`break-${
                            isBreakAll(estimatedAmountResponse.message)
                              ? 'all'
                              : 'words'
                          }`}
                        >
                          {estimatedAmountResponse.message}
                        </span>
                        <button
                    ...
                    >
                      <div className='flex items-center justify-between space-x-2'>
                        <span
                          className={`break-${
                            isBreakAll(swapResponse.message) ? 'all' : 'words'
                          }`}
                        >
                          {swapResponse.message}
                        </span>
                    ...
                    <Alert
                      ...
                      icon={
                        <BiMessageDots className='w-4 sm:w-6 h-4 sm:h-6 stroke-current mr-3' />
                      }
                      closeDisabled={true}
                      rounded={true}
                    >
