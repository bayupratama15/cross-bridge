...
export default function Footer() {
  ...
  return (
    <div
      className={`footer flex flex-col md:flex-row items-center text-xs font-light space-y-2 sm:space-y-0 p-3 ${theme}`}
    >
      <span className='w-full md:w-1/2 lg:w-1/3 flex items-center justify-center md:justify-start text-gray-400'>
        <div className='flex items-center text-gray-600 dark:text-white font-medium space-x-1.5'>
          ...
          >
            ...
            />
            <span>Brewhost Protocol</span>
          </a>
          {_package?.dependencies?.['@connext/nxtp-sdk'] && (
            <span className='font-mono text-gray-400 dark:text-gray-600 font-light'>
              (SDK v
              {_package.dependencies['@connext/nxtp-sdk'].replace('^', '')})
            </span>
          )}
        </div>
      </span>
