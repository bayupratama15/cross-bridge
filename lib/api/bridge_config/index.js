...
const request = async (path, params, options) => {
  const res = await fetch(
    getRequestUrl(process.env.NEXT_PUBLIC_API_URL, path, {
      ...params,
      module: _module,
    }),
    options
...
export const chains = async (params) => {
  ...
  const response =
    process.env.NEXT_PUBLIC_USE_CONFIG === 'connext'
      ? await request(null, params)
      : process.env.NEXT_PUBLIC_NETWORK === 'testnet'
      ? chains_testnet_config
      : chains_config;
  return Array.isArray(response)
    ? response.filter((a) => !a?.is_staging || is_staging)
    : [];
};
  ...
  return Array.isArray(response)
    ? response
        ...
        .map((a) => {
          return {
            ...a,
            contracts: a?.contracts?.filter(
              (c) => !c?.is_staging || is_staging
            ),
          };
        })
    : [];
...
export const announcement = async (params) => {
  ...
  return process.env.NEXT_PUBLIC_USE_CONFIG !== 'connext' &&
...
export const setAnnouncement = async (params, auth) => {
  ...
  return await request(
    ...
    auth && {
      headers: {
        Authorization: base64.encode(`${auth.username}:${auth.password}`),
      },
    }
  );
};
